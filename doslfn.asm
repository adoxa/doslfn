...
tunnel2 	dw	?;0	;Kopie fÅr MOVE-Vorgang \
...
@@1:	;SBCS
	...
	push	di		;Lese-Position
	 ...
	 xor	cx,cx
	 cwd
	 DOS			;Datei-LÑnge in DX:AX, DX wird ignoriert
	pop	dx
	sub	ax,dx
...
;VR: alle
	...
	dec	eax
	push	[CurSector]	;preserve the current sector
	 call	Calc_Next_Cluster ; in case this is the final cluster
	pop	[CurSector]
...
;VR: [sektor] zerstîrt
	...
	;Replace the first entry in the root directory
	mov	eax,[DPB_DirSec]
	call	ReadSecEAX_setBX
	mov	di,ofs DirEnt_Save
	call	copy_dirent
...
proc copy_dirent
;FU: make a copy of the directory entry
;PE: BX=entry
;    DI=destination
;VR: SI,DI,CX=0
	mov	si,bx
...
@@f_eol:
	...
	btr	dx,3		;schon kein DirEnt mehr Platz im Cluster?
				;weiter mit normalem Next_DirEnt
	jnc	@@f_era1	;doch!
	push	dx
	push	[CurSector]
	 call	FlushDirty
	...
	mov	bx,ofs Sektor + length Sektor
	jmp	@@to_next
@@f_oldspace:
	dec	dh
	jns	@@to_next
...
DirEnt_Save	TDirEnt <>	;entry used to extend a directory cluster
...
;VR: -
	...
	dec	edx
	shr	edx,11		;genauer: durch SektorlÑnge (krumm!) teilen
	ret
endp
...
;	call	Alloc_Find_Handle
;	pop	[CurSector]
;	mov	bx,ofs CD_Sektor ;zurÅckstellen!
...
Alt_String_Table =	$+100	;einige Strings im "kritischen Bereich"
...
;Au·erhalb von InstChk: Bit6(CH)=1 wenn irgendein Schalter akzeptiert
		...
		jnz	@@ni
		mov	es,dx
		cmp	[es:MaxCluster],0FFF7h	;check the resident value
		jz	@@T21I		;to check if we're the same DOSLFN
@@ni:
		BSET	ch,bit 7
		mov	dx,ds
@@T21I:
